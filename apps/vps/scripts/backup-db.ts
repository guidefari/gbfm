    ...
    Effect.gen(function* () {
      ...
      if (backupSource === 'local') {
        ...
        const getResourceOrEnv = (resourceKey: string, envKey: string): string => {
          try {
            return (
              (Resource as unknown as Record<string, { value?: string }>)[resourceKey]?.value ||
              process.env[envKey] ||
              ''
            )
          } catch {
      ...
      if (backupData.length < MIN_BACKUP_BYTES) {
        return yield* Effect.fail(
          new Error(
            `Backup integrity check failed: dump is only ${backupData.length} bytes, expected at least ${MIN_BACKUP_BYTES}`
          )
        )
      }

      yield* Console.log(`✅ Backup size: ${(backupData.length / 1024 / 1024).toFixed(2)} MB`)
      ...
      Effect.catch((error) =>
        Console.error(`❌ Backup failed: ${error}`).pipe(Effect.flatMap(() => Effect.fail(error)))
      )
    )
  )
}
