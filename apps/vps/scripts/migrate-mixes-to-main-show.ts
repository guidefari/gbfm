...
interface DatabaseServiceShape {
  ...
  readonly findMixesWithoutShow: () => Effect.Effect<
    ...
    DatabaseError
  >
  readonly updateMixWithShow: (mixId: string, showId: string) => Effect.Effect<void, DatabaseError>
}
      ...
      migrateMixesToMainShow: () =>
        Effect.gen(function* () {
          ...
          if (!mainShow) {
            ...
            yield* Console.log(`Created main show: ${mainShow.title} (${mainShow.id})`)
          } else {
            yield* Console.log(`Found main show: ${mainShow.title} (${mainShow.id})`)
          }
          ...
          for (const mix of mixesToMigrate) {
            const result = yield* Effect.exit(
              db
                ...
                .pipe(Effect.andThen(Console.log(`✅ Migrated mix: ${mix.title ?? mix.slug}`)))
            )
...
Effect.runPromise(
  program.pipe(
    ...
    Effect.catch((error) =>
      Console.error('Migration failed:', error).pipe(Effect.andThen(Effect.fail(error)))
    )
  )
