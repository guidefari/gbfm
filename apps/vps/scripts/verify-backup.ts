...
const downloadLatestBackup = Effect.gen(function* () {
  ...
  yield* Console.log(`✅ Latest: ${latest.Key} (${((latest.Size ?? 0) / 1024 / 1024).toFixed(2)} MB)`);
...
const restoreAndVerify = (filePath: string) =>
  Effect.gen(function* () {
    ...
    if (restore.exitCode !== 0) {
      return yield* Effect.fail(new Error(`psql restore failed (exit ${restore.exitCode}): ${restore.stderr}`));
    }

    const warnings = restore.stderr.split("\n").filter((l) => l && !l.includes("NOTICE") && !l.includes("already exists") && !l.includes("hypopg"));
