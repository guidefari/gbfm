...
export function ReleasesTable({ releases }: ReleasesTableProps) {
  ...
  return (
    <div className='mt-8'>
      <h3 className='mb-4 text-lg font-semibold'>Releases ({releases.length})</h3>
        ...
        <table className='w-full border-collapse'>
          ...
          <tbody>
            {releases.map((release) => (
              <tr key={release.id} className='border-b border-border/50 hover:bg-muted/50'>
                ...
                <td className='px-2 py-3'>
                  {release.streamingLinks && release.streamingLinks.length > 0 ? (
                    <div className='flex flex-wrap gap-1'>
                      {release.streamingLinks.map((link: { platform: string; url: string }) => (
                        <a
