import { type EditorState, RangeSetBuilder, StateField, type Extension } from '@codemirror/state'
...
export function createMusicEntityEditorEmbeds(options: MusicEntityEditorOptions): Extension {
  ...
  const lifecycleListener = EditorView.updateListener.of((update) => {
    ...
    const changes: Array<EditorialMusicDocumentChange> = []
  ...
  const pasteHandler = EditorView.domEventHandlers({
    paste(event, view) {
      ...
      const selections: Array<EditorialMusicSelection> = view.state.selection.ranges.map(
        ({ from, to }) => ({ from, to }),
      )
