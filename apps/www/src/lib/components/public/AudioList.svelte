<script lang="ts">
  ...
  const play = (item: PublicRecord) => window.dispatchEvent(new CustomEvent('gbfm:play-track', { detail: { url: text(item.url), title: text(item.title, 'Episode'), artwork: text(item.thumbnailUrl) || undefined } }))
</script>
