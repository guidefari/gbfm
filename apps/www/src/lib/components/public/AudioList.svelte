<script lang="ts">
  ...
  const play = (item: PublicRecord) => window.dispatchEvent(new CustomEvent('gbfm:play-track', { detail: { id: text(item.id, text(item.url)), slug: text(item.slug), type: 'mix', url: text(item.url), title: text(item.title, 'Episode'), thumbnailUrl: text(item.thumbnailUrl) || null } }))
</script>
