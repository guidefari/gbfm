<script lang="ts">
  ...
  const sections = $derived([
    ...
    { title: 'Editorial', items: records(content?.editorials), href: (entry: PublicRecord) => `/editorial/${entry.slug}` },
    { title: 'Tweets', items: records(content?.tweets), href: (entry: PublicRecord) => `/tweet/${entry.slug}` }
  ])
</script>
...
{:else}
  <div class="mx-auto flex max-w-7xl flex-col gap-8 p-4 lg:flex-row lg:p-8">
    <aside class="w-full shrink-0 lg:w-80">
      ...
      {#if socialLinks.length}<nav aria-label="Social links" class="mt-6 flex flex-col border border-border">{#each socialLinks as link}{#if text(link.url)}<a href={text(link.url)} target="_blank" rel="noopener noreferrer" class="border-b border-border px-3 py-2 text-sm font-bold last:border-0">{text(link.platform, 'Website')} ↗</a>{/if}{/each}</nav>{/if}
    </aside>
    <main class="min-w-0 flex-1 divide-y divide-border">
      {#each sections as section}
        {#if section.items.length}<section class="py-6"><h2 class="mb-4 text-xl font-black text-highlight">{section.title}</h2><ContentGrid items={section.items} href={section.href} /></section>{/if}
      {/each}
      {#if sections.every((section) => section.items.length === 0)}<PublicState message="No public content yet." />{/if}
    </main>
  </div>
{/if}
