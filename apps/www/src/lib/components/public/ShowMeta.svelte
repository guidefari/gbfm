...
<header
  class="grid grid-cols-[6rem_minmax(0,1fr)] items-start gap-4 font-mono sm:grid-cols-[9rem_minmax(0,1fr)] sm:gap-6"
>
  ...
  <div class="min-w-0">
    ...
    {#if text(show.description)}<p
        class="mt-3 hidden max-w-prose text-sm leading-relaxed text-muted-foreground sm:line-clamp-3"
      >
  ...
  {#if text(show.description)}<p
      class="col-span-2 text-sm leading-relaxed text-muted-foreground sm:hidden"
    >
      {text(show.description)}
    </p>{/if}
</header>
