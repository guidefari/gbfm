...
>
  <div class="p-6">
    <div class="flex items-start justify-between gap-4">
      <div>
        ...
        <p id="auth-prompt-description" class="mt-2 text-sm text-muted-foreground">
          {mode === 'choice'
            ? `Keep your ${action === 'favorite' ? 'favorites' : 'show subscriptions'} with a free listener account.`
            : 'You will stay on this page when you are done.'}
        </p>
      </div>
      <button type="button" class="px-2 text-xl" aria-label="Close" onclick={close}>×</button>
    </div>

    {#if error}<p role="alert" class="mt-4 border border-destructive p-3 text-sm text-destructive">
        {error}
      </p>{/if}
    {#if verificationEmail}
      <div class="mt-6" role="status">
        ...
        <p class="mt-2 text-sm text-muted-foreground">
          We sent a verification link to {verificationEmail}. After verifying, return here to
          continue.
        </p>
      </div>
    {:else if mode === 'choice'}
      <div class="mt-6 grid gap-3">
        <button
          class="bg-primary px-4 py-3 font-bold text-primary-foreground"
          onclick={() => (mode = 'sign-in')}>Sign in</button
        >
        <button class="border border-border px-4 py-3 font-bold" onclick={() => (mode = 'sign-up')}
    ...
    {:else}
      ...
      >
        ...
        <button
          ...
          disabled={pending}
          >{pending ? 'Working…' : mode === 'sign-in' ? 'Sign in' : 'Create account'}</button
        >
        <button
          type="button"
    ...
    {/if}
    <noscript
      ><a href={`/auth/sign-in?redirect=${encodeURIComponent(returnPath)}`}>Sign in</a></noscript
    >
  </div>
</dialog>
