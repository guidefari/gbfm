...
<dialog
  ...
  aria-label="Search"
  class="m-0 mx-auto mt-[12vh] w-[calc(100%-2rem)] max-w-xl rounded-sm border border-border bg-background p-0 text-foreground shadow-2xl backdrop:bg-black/70"
>
  <form
    role="search"
    onsubmit={(event) => {
  ...
  >
    ...
    <input
      ...
      autocomplete="off"
      class="h-12 min-w-0 flex-1 bg-transparent text-base outline-none placeholder:text-muted-foreground/70"
    />
    <button
      type="button"
      ...
      {:else if results}
        {#each groups as group (group.key)}
          ...
          {#if items.length > 0}
            <section>
              <h3
                class="bg-muted/40 px-4 py-1 text-[11px] font-semibold tracking-wide text-muted-foreground"
              >
                {group.label}
              </h3>
              {#each items as { result, href } (result.id)}
                <a
                  {href}
                  class="flex items-center gap-3 border-b border-border/30 px-4 py-2 no-underline last:border-b-0 hover:bg-muted/50 focus-visible:bg-muted/50 focus-visible:outline-none"
                >
                  {#if result.thumbnailUrl}<img
                  ...
                  <span class="min-w-0 flex-1">
                    <span class="block truncate text-sm text-foreground"
                      >{result.title || result.description?.slice(0, 80) || '(untitled)'}</span
                    >
                    {#if result.description && result.title}<span
                        class="block truncate text-xs text-muted-foreground"
                        >{result.description}</span
                      >{/if}
                  </span>
                  <span
                    class="shrink-0 rounded-sm bg-muted px-1.5 py-0.5 text-[10px] font-semibold text-muted-foreground"
                    >{searchResultLabel(result.type)}</span
                  >
                </a>
              {/each}
            </section>
  ...
  {:else}
    <p class="px-4 py-3 text-xs text-muted-foreground">
      Press Enter to open the top result. <kbd class="font-mono">Esc</kbd> to close.
    </p>
  {/if}
</dialog>
