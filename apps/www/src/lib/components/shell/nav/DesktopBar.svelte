...
<div class="hidden h-full items-center gap-4 px-4 lg:flex lg:pr-6">
  <a
    ...
    class="flex shrink-0 items-center text-foreground no-underline transition-colors hover:text-highlight"
  ...
  <div class="flex shrink-0 items-center gap-1">
    {#each desktopNavItems as item (item.id)}
      <a
        ...
        class="rounded-sm px-2 py-1 text-xs font-semibold tracking-wide text-muted-foreground no-underline transition-colors hover:text-foreground aria-[current=page]:text-highlight"
        >{item.label}</a
      >
    {/each}
  </div>
  <button
    type="button"
    onclick={onSearch}
    class="flex h-7 items-center gap-2 rounded-sm border border-border px-2 text-xs text-muted-foreground transition-colors hover:border-foreground/40 hover:text-foreground"
  >
    <Search size={12} />
    <span>Search</span>
