...
<div class="max-w-2xl space-y-6">
  <section class="border border-border">
    ...
    <form class="space-y-6 p-6" onsubmit={(event) => { event.preventDefault(); void saveProfile() }}>
      <div class="flex justify-center">
        <label class="group relative h-20 w-20 cursor-pointer overflow-hidden rounded-sm" aria-label="Change avatar">
          {#if imagePreview || image}<img src={imagePreview || image} alt="User Avatar" class="h-20 w-20 object-cover" />{:else}<span class="flex h-20 w-20 items-center justify-center bg-muted text-2xl text-muted-foreground">{(username || email || '?').charAt(0).toUpperCase()}</span>{/if}
          <span class="absolute inset-0 flex items-center justify-center bg-black/60 text-xs text-white opacity-0 transition-opacity group-hover:opacity-100">Change</span>
          <input class="sr-only" type="file" accept="image/*" onchange={(event) => selectAvatar(event.currentTarget.files?.[0])} />
        </label>
      </div>
  ...
  <section class="border border-border"><header class="border-b border-border p-6"><h2 class="text-xl font-bold">Change Password</h2></header><div class="space-y-4 p-6"><p class="text-base text-muted-foreground">We'll send a password reset link to <strong class="text-foreground">{email}</strong>.</p><button type="button" class="border border-border px-4 py-2 font-bold disabled:opacity-50" disabled={resetPending || !email} onclick={() => void resetPassword()}>{resetPending ? 'Sending...' : 'Send Reset Link'}</button></div></section>
  {#if message}<p class:text-destructive={failed} role={failed ? 'alert' : 'status'}>{message}</p>{/if}
</div>
