...
<div class="space-y-8">
  <div class="space-y-1">
    <h2 class="flex items-center gap-2 text-base font-bold tracking-widest text-muted-foreground">
      <span class="text-[#1db954]" aria-hidden="true">●</span> Spotify Connection
    </h2>
    ...
    <p class="text-xs font-medium tracking-wider text-muted-foreground/70">
      Redirect URI: <code>{getSpotifyRedirectUri()}</code>
    </p>
  </div>
  {#if pending}<p class="text-xs font-medium tracking-wider text-muted-foreground">
      Checking session…
    </p>{:else if session}<div class="w-full max-w-md space-y-4">
      <div class="flex items-center justify-between gap-6 border-2 border-border p-6">
        <div class="min-w-0 space-y-2">
          <strong class="block truncate text-base tracking-widest"
            >{profile?.display_name ?? profile?.id ?? 'Connected'}</strong
          ><span class="text-xs text-muted-foreground">Token expires {expiresIn}</span>
        </div>
        <span class="text-xs font-bold tracking-widest text-green-500">Connected</span>
      </div>
      <div class="flex gap-2">
        <button class="border border-border px-3 py-1.5 text-sm" onclick={() => void refresh()}
          >Refresh session</button
        ><button class="px-3 py-1.5 text-sm" onclick={() => void disconnect()}>Disconnect</button>
      </div>
    </div>{:else}<button
      class="w-full max-w-md bg-foreground px-4 py-2 font-bold text-background disabled:opacity-50"
      disabled={connecting}
      onclick={() => void connect()}>{connecting ? 'Connecting…' : 'Connect Spotify'}</button
    >{/if}
