<script lang="ts">
  ...
  function applyTheme(value: Theme) {
    ...
    document
      ...
      ?.setAttribute('content', resolved === 'dark' ? '#16415a' : '#e8eef7')
  }
  ...
  <div class="flex flex-col gap-6 md:flex-row">
    {#each options as option}
      <button
        ...
        class:border-primary={theme === option.value}
        class:bg-muted={theme === option.value}
        class:text-foreground={theme === option.value}
        class="relative flex flex-1 flex-col gap-2 rounded-none border-2 border-border p-6 text-left text-muted-foreground transition-all duration-300 hover:border-primary/50"
      >
        {#if theme === option.value}<span
            class="absolute right-4 top-4 text-primary"
            aria-hidden="true">✓</span
          >{/if}
        <span class="text-base font-bold tracking-widest">{option.title}</span>
        <span class="text-xs font-medium leading-relaxed tracking-wider opacity-70"
