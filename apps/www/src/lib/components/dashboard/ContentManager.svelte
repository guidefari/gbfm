  ...
  async function setDraft(slugs: ReadonlyArray<string>, draft: boolean) {
    ...
    try {
      await Promise.all(
        slugs.map((slug) =>
          dashboardCommand(
            `${actionBase}/${encodeURIComponent(slug)}`,
...
<Page {title} {description}>
  <div class="flex flex-wrap gap-3 rounded border p-4">
    {#if !isAudio}
      ...
      <select
        ...
        onchange={(event) => {
          const decoded = Option.getOrNull(
            Schema.decodeUnknownOption(Status)(event.currentTarget.value),
          )
          if (decoded) {
            status = decoded
            offset = 0
      ...
      >
        <option value="all">All states</option><option value="draft">Drafts</option><option
          value="live">Published</option
        >
      </select>
    {/if}
    <a
      class="rounded bg-foreground px-4 py-2 text-background no-underline"
      href={isAudio ? '/mix-upload' : `/new?mode=${contentType === 'post' ? 'editorial' : 'tweet'}`}
      >Create new</a
    >
  </div>

  {#if selected.length > 0}
    <div class="flex items-center gap-3 rounded border-2 border-highlight bg-highlight/10 p-3">
      <strong>{selected.length} selected</strong><button
        class="underline"
        disabled={pending}
        onclick={() =>
          void setDraft(
            items.filter((item) => selected.includes(item.id)).map((item) => item.slug),
            false,
          )}>Publish</button
      ><button
        class="underline"
  ...
  {#if pending && items.length === 0}<p>Loading…</p>{:else if items.length === 0}<p
      class="rounded border p-6 text-muted-foreground"
    >
    ...
    <div class="overflow-x-auto rounded border">
      <table class="w-full text-left text-sm">
        <thead class="bg-muted"
          ><tr
            ><th class="p-3"><span class="sr-only">Select</span></th><th>Title</th><th>Slug</th><th
              >Status</th
            ><th>Updated</th><th>Actions</th></tr
          ></thead
        ><tbody>
          {#each items as item}<tr class="border-t"
              ><td class="p-3"
                ><input
                ...
                /></td
              ><td class="max-w-80 truncate font-semibold">{item.title || 'Untitled'}</td><td
                class="font-mono text-xs">{item.slug}</td
              ><td>{item.draft ? 'Draft' : 'Published'}</td><td
                >{new Date(item.updatedAt).toLocaleDateString()}</td
              ><td class="whitespace-nowrap"
                ><a class="mr-3 underline" href={editHref(item)}>Edit</a><button
                  class="underline"
                  disabled={pending}
                  onclick={() => void setDraft([item.slug], !item.draft)}
                  >{item.draft ? 'Publish' : 'Unpublish'}</button
                ></td
              ></tr
            >{/each}
        </tbody>
  ...
  <div class="flex justify-between">
    ...
    ><span
      >{total === 0 ? 0 : offset + 1}–{Math.min(total, offset + items.length)} of {total}{scope ===
      'own'
        ? ' yours'
        : ''}</span
    ><button
