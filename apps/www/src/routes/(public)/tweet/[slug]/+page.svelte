...
<PublicHead
  ...
  description={text(
    ...
    text(data.item?.content, 'A tweet on goosebumps.fm'),
  ).slice(0, 160)}
...
{:else}
  ...
  <div class="mx-auto max-w-3xl px-4 py-8">
    ...
    {#if data.parent && !rootIsCurrent}
      ...
      <div class="pb-4">
        <article
          class="space-y-3 overflow-hidden rounded-lg border border-border/40 bg-card p-3 opacity-80 transition-opacity hover:opacity-100"
        >
          <TweetAuthorRow
            creator={records(data.parent.creators)[0] ?? null}
            createdAt={text(data.parent.createdAt)}
            interactive={false}
          />
          <a href={`/tweet/${text(data.parent.slug)}`} class="block no-underline">
            <p class="mt-2 truncate text-base text-muted-foreground">
              {text(data.parent.content, text(data.parent.title))}
            </p>
          </a>
          {#if parentMusic}
            <TweetMusicCard
              type={text(data.parent.musicEntityType)}
              entity={record(parentMusic.entity)}
              links={records(parentMusic.links)}
            />
          {/if}
        </article>
    ...
    <article class="space-y-4 rounded-lg border border-border/60 bg-card/60 p-4 shadow-sm sm:p-5">
      ...
      {#if itemMusic}
        <TweetMusicCard
          type={text(data.item.musicEntityType)}
          entity={record(itemMusic.entity)}
          links={records(itemMusic.links)}
        />
      {/if}
      ...
      {#if data.quote}
        ...
        <article
          class="not-prose space-y-3 overflow-hidden rounded-md border border-border/50 bg-muted/20 p-3 transition-colors hover:bg-muted/30"
        >
          <TweetAuthorRow
            creator={records(data.quote.creators)[0] ?? null}
            createdAt={text(data.quote.createdAt)}
            interactive={false}
          />
          <a href={`/tweet/${text(data.quote.slug)}`} class="block no-underline">
            <p class="mt-2 truncate text-base text-muted-foreground">
              {text(data.quote.content, text(data.quote.title))}
            </p>
          </a>
          {#if quoteMusic}
            <TweetMusicCard
              type={text(data.quote.musicEntityType)}
              entity={record(quoteMusic.entity)}
              links={records(quoteMusic.links)}
            />
          {/if}
        </article>
      {/if}
    ...
    <section id="replies" class="mt-6 scroll-mt-4 space-y-4">
      {#if data.principal._tag === 'Anonymous'}
        <p class="text-base text-muted-foreground">
          <a
            href={`/auth/sign-in?redirect=${encodeURIComponent(`/tweet/${params.slug}`)}`}
      ...
      {:else if !replyOpen}
        <button
          ...
          onclick={() => (replyOpen = true)}>Reply</button
        >
      {:else}
        <form
        ...
        >
          ...
          <div class="flex items-center justify-end gap-2">
            <button
              ...
              onclick={() => (replyOpen = false)}>Cancel</button
            >
            <button
      ...
      <div>
        {#each data.replies as reply, index}
          ...
          <div class="relative">
            {#if index < data.replies.length - 1}<div
            ...
            >
              <TweetAuthorRow
                creator={records(reply.creators)[0] ?? null}
              ...
              >
                <RichContent content={text(reply.content)} />
              </div>
              {#if replyMusic}
                <TweetMusicCard
                  type={text(reply.musicEntityType)}
                  entity={record(replyMusic.entity)}
                  links={records(replyMusic.links)}
                />
              {/if}
              <a
                href={`/tweet/${text(reply.slug)}`}
