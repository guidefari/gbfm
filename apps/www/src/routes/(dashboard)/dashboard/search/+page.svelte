<script lang="ts">
  ...
  async function search() { if (query.trim().length < 2) return; pending = true; failed = false;

 try { results = await dashboardJson(Results, `/api/search?q=${encodeURIComponent(query.trim())}&limit=50`) } catch { failed = true } finally { pending = false } }
</script>
