<script lang="ts">
  ...
  async function submit() {
    ...
    try {
      const response = await fetch('/api/invite/confirm', {
        method: 'POST',
        credentials: 'include',
...
{#if !token || linkError}
  <section class="mx-auto max-w-xl px-4 py-20">
    ...
    {#if linkError}<p role="alert" class="mt-5 border border-destructive p-3 text-destructive">
        Invalid or expired reset link. Please request a new password reset.
      </p>{/if}
    <p class="mt-5 text-muted-foreground">
      Reset links expire for safety. The newest link in your inbox is the one to use.
    </p>
    <a class="mt-6 inline-block font-semibold underline" href="/auth/forgot-password"
      >Request another reset email</a
    >
  </section>
{:else}
  <section class="mx-auto max-w-xl px-4 py-16">
    ...
    {#if data.principal._tag === 'Authenticated'}<p
        class="mt-6 border border-amber-500/50 bg-amber-500/10 p-3"
        role="status"
      >
        You’re currently signed in as {data.principal.email}. Setting a password here will switch to
        the account from the reset link.
      </p>{/if}
    {#if error}<p class="mt-4 border border-destructive p-3 text-destructive" role="alert">
        {error}
      </p>{/if}
    <form
    ...
    >
      ...
      <ul class="grid gap-1 text-xs">
        {#each checks as [label, passed]}<li
            class={passed ? 'text-highlight' : 'text-muted-foreground'}
          >
            {passed ? '✓' : '○'}
            {label}
          </li>{/each}
      ...
      <button
        ...
        disabled={pending || !valid}>{pending ? 'Resetting…' : 'Reset password'}</button
      >
    </form>
