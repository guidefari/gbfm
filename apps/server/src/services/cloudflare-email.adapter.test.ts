...
] satisfies ReadonlyArray<readonly [CloudflareRejectedProviderCode, EmailRejectionReason]>

describe('CloudflareEmailTransportLayer', () => {
  test('translates a complete message through the public transport interface', async () => {
    const messages: Array<CloudflareEmailMessage> = []
  ...
  test.each(rejectionCases)(
    'maps documented %s to the safe %s rejection category',
    async (code, reason) => {
      const binding: CloudflareEmailBinding = {
      ...
      expect(error).toMatchObject(new EmailRejected({ reason, providerCode: code }))
    },
  )
  ...
  test('classifies unknown thrown values and empty receipts as unavailable without raw causes', async () => {
    ...
    expect(unavailable).toMatchObject(new EmailUnavailable({ providerCode: 'unknown' }))
    expect(unavailable).not.toHaveProperty('cause')
    expect(invalidReceipt).toMatchObject(new EmailUnavailable({ providerCode: 'invalid-receipt' }))
  })
  ...
  test('does not classify inherited properties such as toString as a provider code', async () => {
    ...
    await expect(Effect.runPromise(Effect.flip(send(binding)))).resolves.toMatchObject(
      new EmailUnavailable({ providerCode: 'unknown' }),
    )
  })
})
