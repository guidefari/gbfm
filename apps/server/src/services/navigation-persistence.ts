...
type Snapshot = {
  ...
  readonly neighbours: NavigationResult['neighbours']
  readonly neighbourhood: NavigationResult['neighbourhood']
}
...
export const makeNavigationPersistence = Effect.gen(function* () {
  ...
  // destination. Persisted results are bounded by the destination in the trail.
  const resultSnapshot = (
    target:
      | { readonly sessionId: string; readonly position: number }
      | {
    ...
    const currentPostCreatedAt =
      'from' in target
        ? db
            ...
            .from(postsTable)
            .where(and(eq(postsTable.slug, target.from), eq(postsTable.type, 'micro')))
            .limit(1)
        : db
            .select({ createdAt: postsTable.createdAt })
            .from(navigationTrailEntries)
  ...
  const sessionIdsForIdentity = (identity: NavigationIdentity) =>
    db.select({ id: navigationSessions.id }).from(navigationSessions).where(identityWhere(identity))
  ...
  return {
    ...
    attempt,
    read,
    reset: (identity: NavigationIdentity) =>
      Effect.gen(function* () {
