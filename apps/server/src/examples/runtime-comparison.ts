...
const ExpensiveServiceLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.log(`[${++initCount}] Initializing service... (expensive!)`)
...
// ==========================================
export async function approachWithoutRuntime() {
  await Effect.runPromise(Effect.log('\n=== WITHOUT REUSED CONTEXT ==='))
  ...
  await Effect.runPromise(Effect.log(`Time: ${duration.toFixed(2)}ms`))
  await Effect.runPromise(Effect.log(`Initializations: ${initCount}, Disposals: ${disposeCount}`))
}

// ==========================================
// APPROACH 2: Reused Context
// ==========================================
export async function approachWithRuntime() {
  await Effect.runPromise(Effect.log('\n=== WITH REUSED CONTEXT ==='))
  initCount = 0
  disposeCount = 0
