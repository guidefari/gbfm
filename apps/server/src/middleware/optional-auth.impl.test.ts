...
describe('IdentityResolver', () => {
  it('mints an anonymous device token and sets it in a secure httpOnly cookie', async () => {
    ...
    expect(Predicate.isTagged('Anonymous')(identity)).toBe(true)
    expect(identity).toHaveProperty('deviceToken', expect.any(String))
  ...
  it('uses an existing anonymous device token without setting a replacement cookie', async () => {
    ...
    const identity = await response.json()
    expect(Predicate.isTagged('Anonymous')(identity)).toBe(true)
    expect(identity).toHaveProperty('deviceToken', 'known-device')
