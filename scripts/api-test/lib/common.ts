...
export function printHeaders(headers: Headers, verbose = false): void {
  ...
  const interestingHeaders = ['location', 'content-type', 'cache-control', 'set-cookie']

  headers.forEach((value, key) => {
    const keyLower = key.toLowerCase()
    if (verbose || interestingHeaders.includes(keyLower) || keyLower.startsWith('x-')) {
...
export function parseArgs(args: string[]): {
  ...
  const positional = args.filter((a) => !a.startsWith('-') && a !== '-v' && a !== '--verbose')
