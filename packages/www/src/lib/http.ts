...
export const signin = async (email: string): RedirectUrl => {
  ...
  console.log(
    "https://auth.guidefari.staging.goosebumps.fm" + "/code/authorize?" + params
  )

  return `https://auth.guidefari.staging.goosebumps.fm/code/authorize?${params}`
  //   return redirect(Resource.AuthRouter.url + "/code/authorize?" + params)
}

export const authCallback = async (code: string) => {
  return redirect(
    "https://auth.guidefari.staging.goosebumps.fm" +
      "/code/callback?" +
