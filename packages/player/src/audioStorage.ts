...
const parseJson = (raw: string): Effect.Effect<Schema.Json, AudioStorageError> =>
  Effect.try({
...
export const createAudioStorage = (adapter: AudioStorageAdapter, now: () => number = Date.now) => {
  const read = (key: string): Effect.Effect<string | null, AudioStorageError> =>
    Effect.tryPromise({
      try: () => adapter.read(key),
      catch: (cause) => new AudioStorageError('read', cause),
    })

  const write = (key: string, value: string): Effect.Effect<void, AudioStorageError> =>
    Effect.tryPromise({
      try: () => adapter.write(key, value),
      catch: (cause) => new AudioStorageError('write', cause),
    })

  const remove = (key: string): Effect.Effect<void, AudioStorageError> =>
    Effect.tryPromise({
      try: () => adapter.remove(key),
      catch: (cause) => new AudioStorageError('delete', cause),
    })

  const loadQueue = (): Effect.Effect<PersistedQueueType | null, AudioStorageError> =>
    Effect.gen(function* () {
  ...
  const saveQueue = (queue: PersistedQueueType): Effect.Effect<void, AudioStorageError> =>
    write(QUEUE_KEY, JSON.stringify(queue))

  const loadPosition = (
    trackId: string,
  ): Effect.Effect<PositionRecordType | null, AudioStorageError> =>
    Effect.gen(function* () {
  ...
  const clearPosition = (trackId: string): Effect.Effect<void, AudioStorageError> =>
    remove(keyForTrack(POSITION_KEY_PREFIX, trackId))

  const loadPlay = (
    trackId: string,
  ): Effect.Effect<{ readonly at: number } | null, AudioStorageError> =>
    Effect.gen(function* () {
  ...
  const recordPlay = (trackId: string): Effect.Effect<void, AudioStorageError> =>
    write(keyForTrack(PLAY_KEY_PREFIX, trackId), JSON.stringify({ at: now() }))

  const isWithinDedupWindow = (trackId: string): Effect.Effect<boolean, AudioStorageError> =>
    Effect.gen(function* () {
  ...
  const loadVolume = (): Effect.Effect<VolumeRecordType | null, AudioStorageError> =>
    Effect.gen(function* () {
  ...
  const saveVolume = (volume: VolumeRecordType): Effect.Effect<void, AudioStorageError> =>
