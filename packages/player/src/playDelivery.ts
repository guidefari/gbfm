...
type PlayDeliveryDependencies<CheckError, DeliveryError, RememberError> = {
  readonly isWithinDedupWindow: (trackId: string) => Effect.Effect<boolean, CheckError>
  readonly deliver: (trackId: string) => Effect.Effect<void, DeliveryError>
  readonly remember: (trackId: string) => Effect.Effect<void, RememberError>
  readonly now: () => number
}
