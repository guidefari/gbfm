...
### 1. `packages/email/emails/invite.tsx` *(new)*
...
### 5. `apps/vps/src/routes/invite/invite.handlers.ts` *(new)*
Handler logic:
...
### 6. `apps/vps/src/routes/invite/invite.index.ts` *(new)*
Wire route to handler via `createRouter().openapi(...)`.

### 7. `apps/vps/src/app.ts` (or wherever routes are mounted) *(modify)*
Mount `/invite` router.

### 8. `apps/www/src/routes/admin/_components/UsersTab.tsx` *(modify)*
