...
| 6 | 🔵 in progress (search #156, profile #157, resolve #158 merged) | Port remaining JSON groups incrementally, one group per PR/deploy | Low-Medium | Mechanical work, same shape as step 4 each time | One group per PR |
| 6b | ⬜ not started | Replace the manual `fetcher()` calls in `apps/www/src/lib/http.ts` for each step-6 group (search, profile, resolve, ...) with the typed `HttpApiClient`, same pattern as step 5's music-artists hook swap | Low | Closes the loop step 5 opened -- step 6 has been landing server-side ports without touching the client, so `apps/www` is accumulating groups it could already consume through the typed client but doesn't yet | One hook per PR, or a small batch of same-shaped hooks (e.g. `useProfile` + `useResolveSlug`) if they're trivial |
